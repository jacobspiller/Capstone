...
#define CMD_BUFF_SIZE 512
  ...
  #define LOCAL_NAME  'S','J','o','r','t','_','1','2'
...
uint8_t SerialPort_DeviceInit(void)
{
  ...
  if(ret != BLE_STATUS_SUCCESS){
    SEGGER_RTT_printf(0,"Setting BD_ADDR failed: 0x%02x\r\n", ret);
    return ret;
  }
  ...
  ret = aci_gatt_init();
  if (ret != BLE_STATUS_SUCCESS) {
	  SEGGER_RTT_printf(0,"Error in aci_gatt_init(): 0x%02x\r\n", ret);
    return ret;
  } else {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    SEGGER_RTT_printf(0,"Error in aci_gap_init() 0x%02x\r\n", ret);
    return ret;
  } else {
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    SEGGER_RTT_printf(0,"Error in Gatt Update characteristic value 0x%02x\r\n", ret);
    return ret;
  } else {
    SEGGER_RTT_printf(0,"aci_gatt_update_char_value_ext() --> SUCCESS\r\n");
  }

#if  SERVER
  ret = Add_SerialPort_Service();
  if (ret != BLE_STATUS_SUCCESS) {
    SEGGER_RTT_printf(0,"Error in Add_SerialPort_Service 0x%02x\r\n", ret);
    return ret;
  } else {
    SEGGER_RTT_printf(0,"Add_SerialPort_Service() --> SUCCESS\r\n");
  }
  
#if ST_OTA_FIRMWARE_UPGRADE_SUPPORT     
  ret = OTA_Add_Btl_Service();
  if(ret == BLE_STATUS_SUCCESS)
    SEGGER_RTT_printf(0,"OTA service added successfully.\n");
  else
    SEGGER_RTT_printf(0,"Error while adding OTA service.\n");
...
{  
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    SEGGER_RTT_printf(0,"Error while starting connection: 0x%04x\r\n", ret);
    Clock_Wait(100);        
  }
  ...
  if(ret != BLE_STATUS_SUCCESS)
  {
    SEGGER_RTT_printf(0,"Error in aci_gap_set_discoverable(): 0x%02x\r\n", ret);
  }
  else
  {
...
void APP_Tick(void)
{
  ...
  if (APP_FLAG(CONNECTED) && !APP_FLAG(END_READ_TX_CHAR_HANDLE))
  {
    if (!APP_FLAG(START_READ_TX_CHAR_HANDLE))
    {
      ...
      if (ret != 0) 
        SEGGER_RTT_printf(0,"Error in aci_gatt_disc_char_by_uuid() for TX characteristic: 0x%04xr\n", ret);
      else
        SEGGER_RTT_printf(0,"aci_gatt_disc_char_by_uuid() for TX characteristic --> SUCCESS\r\n");
  ...
  else if (APP_FLAG(CONNECTED) && !APP_FLAG(END_READ_RX_CHAR_HANDLE))
  {
    ...
    if (!APP_FLAG(START_READ_RX_CHAR_HANDLE))
    {
      ...
      if (ret != 0) 
        SEGGER_RTT_printf(0,"Error in aci_gatt_disc_char_by_uuid() for RX characteristic: 0x%04xr\n", ret);
      else
        SEGGER_RTT_printf(0,"aci_gatt_disc_char_by_uuid() for RX characteristic --> SUCCESS\r\n");
  ...
  attr_handle = Attribute_Handle;
    if(attr_handle == tx_handle+1)
    {
      for(int i = 0; i < Attribute_Value_Length; i++) 
          SEGGER_RTT_printf(0,"%c", Attribute_Value[i]);
    }
}
...
{ 
  if (APP_FLAG(START_READ_TX_CHAR_HANDLE) && !APP_FLAG(END_READ_TX_CHAR_HANDLE))
  {
    SEGGER_RTT_printf(0,"aci_GATT_PROCEDURE_COMPLETE_Event for START_READ_TX_CHAR_HANDLE \r\n");
